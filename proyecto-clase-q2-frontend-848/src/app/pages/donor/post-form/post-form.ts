...
export class PostFormComponent implements OnInit {
  ...
  // Para mostrar error de foto en el template
  get noPhotosError(): boolean {
    return this.submitAttempted && this.photoUrls.length === 0;
  }
    ...
    files.forEach(file => {
      this.uploadService.uploadPhoto(file).subscribe({
        ...
        error: (err) => {
          ...
          this.snackBar.open(
            err.error?.message || 'Error al subir foto',
            'OK',
            { duration: 4000 }
          );
        }
  ...
  onSubmit(): void {
    ...
    // Bloquear si no hay fotos
    if (this.photoUrls.length === 0) {
      this.snackBar.open(
        'Debes subir al menos una foto del artículo',
        'OK',
        { duration: 4000 }
      );
      this.cdr.detectChanges();
    ...
    this.postService.create(data).subscribe({
      ...
      error: (err) => {
        ...
        this.cdr.detectChanges();
        this.snackBar.open(
          err.error?.message || 'Error al crear publicación',
          'OK',
          { duration: 4000 }
        );
      }
